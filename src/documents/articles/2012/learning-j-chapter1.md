...
### 1.1 인터렉티브 환경
J의 개발환경은 사용자가 한 줄 입력하면, 그게 곧바로 해석되어 아랫줄에 출력된다. 사용자의 입력은 기본적으로 들여쓰기가 되어있고, 아래 줄에 출력되는 결과는 들여쓰기가 없다.
...
결과는 똑같다. 빈칸은 있어도 되고 없어도 되는 것이다. 가독성을 위해서 필요하다면 넣으면 된다.
...
다음 예제는 음수에 대한 것이다. 음수는 숫자 앞에 `_`부호가 붙는다. 숫자와 `_`사이에는 빈칸이 있어선 안 된다. `_`는 산수 할 때 쓰는 연산자가 아니라 숫자의 표현 중 하나이다. 마치 부동소수점을 표현할 때 수 중간에 `.`을 찍는 것과 같은 것이다.
...
### 1.8 내장 함수들 더 보기
이 섹션의 목적은 J의 내장 함수 중 몇 개를 더 소개해서 J로 프로그래밍한다는 것의 맛을 좀 느끼게 하는 것이다.
...
이 표는 코드가 수행되는 순서에 따라서, `x`를 입력하면 `5 4 1 9`가 나오고, `x=x`를 입력하면 `1 1 1 1`이 나오고... 그런 의미이다. 이런 표시방식은 J에서 제공하는 것은 아니다. 하지만 여기에선 자주 쓰일 것이다. 첫째 줄에는 입력들이, 그 아래 줄에는 출력들이 나온다.

할당문(`x=:something`)을 입력했을 때에는 아무것도 표시되지 않은 것을 봤을 것이다. 그래도 할당문은 표현식이기 때문에 값을 가진다. 할당문의 값을 보거나, 뭔가를 기억하거나 보기에 쉽도록 종종 차례로 문장이 완성되는 것을 보여줄 것이다. 예를 들자면 아래와 같다.
